...
library(readr)
artists <- read_csv("~/Smith/Columbia/AppliedDS/fall2019-proj1--NicholeY/data/artists.csv")
```
...
stop_words <- c(stop_words$word, word)
# clean the data and make a corpus
corpus <- VCorpus(VectorSource(artists$Intro))%>%
  tm_map(content_transformer(tolower))%>%
  tm_map(removePunctuation)%>%
  tm_map(removeWords, character(0))%>%
  tm_map(removeWords, stop_words)%>%
...
```{r export data}
save(artists, file="../output/processed_artists.RData")
```
